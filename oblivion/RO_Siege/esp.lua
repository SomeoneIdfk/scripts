--[[
    ...
    - iRay#1488: Chams
    Edited for Roblox Operations: Siege
    - SomeoneIdfk
]]
...
local MapInfoSuccess, MapInfo = pcall(function() return workspace.SE_Workspace end)
...
function EspLibrary.GetDroneBoundingBox(torso)
    local torsoPosition, onScreen, depth = worldToViewportPoint((torso.CFrame * CFrame.new(EspLibrary.options.offsetX,EspLibrary.options.offsetY,0)).p)
    local scaleFactor = 1 / (tan(rad(currentCamera.FieldOfView * 0.5)) * 2 * depth) * 500
    local size = round(vector2New(EspLibrary.options.scaleFactorX * scaleFactor, EspLibrary.options.scaleFactorY * scaleFactor))
    return onScreen, size, round(vector2New(torsoPosition.X - (size.X * 0.5), torsoPosition.Y - (size.Y * 0.5))), torsoPosition
end
...
function EspLibrary.AddDroneEsp(drone)
    if (players:FindFirstChild(tostring(drone.Owner.Value)) == localPlayer) then
        return
    end
    ...
    EspLibrary.espDroneCache[drone] = objects
end
...
function EspLibrary.RemoveDroneEsp(drone)
    local espDroneCache = EspLibrary.espDroneCache[drone]

    if (espDroneCache) then
        EspLibrary.espDroneCache[drone] = nil

        for _, object in next, espDroneCache do
            object:Remove()
        end
    end
end
...
function EspLibrary.Init()
    ...
    if MapInfoSuccess then
        insert(EspLibrary.conns, MapInfo.Drones.ChildAdded:Connect(function(drone)
            repeat wait(1) until drone:FindFirstChild("Owner")
            EspLibrary.AddDroneEsp(drone)
        end))

        insert(EspLibrary.conns, MapInfo.Drones.ChildRemoved:Connect(function(drone)
            EspLibrary.RemoveDroneEsp(drone)
        end))
            
        for _, drone in next, MapInfo.Drones:GetChildren() do
            EspLibrary.AddDroneEsp(drone)
        end
    end
    ...
    runService:BindToRenderStep("esp_rendering", Enum.RenderPriority.Camera.Value + 1, function()
                ...
                if EspLibrary.options.chams then
                    if findFirstChild(torso, "BoxHandleAdornment") then
                        for _, part in next, character:GetChildren() do
                            ...
                            elseif findFirstChild(part, "BoxHandleAdornment") == false and string.find(part.ClassName, "Part") then
        ...
        if MapInfoSuccess then
            for drone, objects in next, EspLibrary.espDroneCache do
                local character, torso = EspLibrary.GetDrone(drone)
    
                if (character and torso) then
                    local onScreen, size, position, torsoPosition = EspLibrary.GetDroneBoundingBox(torso)
                    local distance = EspLibrary.GetDistance(torso.Position)
                    local canShow = onScreen and (size and position) and EspLibrary.options.enabled
                    local player = players:FindFirstChild(tostring(drone.Owner.Value))
                    if player then
                        local team, teamColor = EspLibrary.GetTeam(player)
                        local color = EspLibrary.options.teamColor and teamColor or nil
